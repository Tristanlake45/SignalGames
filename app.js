...
(() => {
    ...
    const ANSWER_SET = new Set(ANSWER_WORDS);
    ...
    async function loadValidWordsTxtOrFail() {
      ...
      // Normalize + sanitize: only A-Z exactly 5 letters
      const sanitized = sanitizeWordList(lines);
  
      // Make sure all answers are guessable too
      const merged = sanitizeWordList([...sanitized, ...ANSWER_WORDS]);
  
      VALID_GUESS_SET = new Set(merged);
  
      if (VALID_GUESS_SET.size < 500) {
        // not a hard error, but gives you a clear “robustness” signal
        console.warn(`valid-words.txt seems small: ${VALID_GUESS_SET.size} words. Consider expanding it.`);
      }
    ...
    function submitGuess() {
      ...
      // ✅ Robust dictionary check from valid-words.txt
